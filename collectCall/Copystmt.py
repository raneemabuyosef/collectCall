...
class Copy:
    def __init__(self, lineno, send_expr, send_gexpr, recv_expr, recv_gexpr):
        self.filename = f'{lineno:03d}copy.py'
...
#            if self.functionID == 2:
 #               break
    ...
    #    constraints += [f"STATE({sendTensorName}, {sendexprISLDims}, {sendgexprISLDims}, {sendtime}) == {holder.getState()}"]
        ...
        constraints += [f"STATE({sendTensorName}, {sendexprISLDims}, {sendgexprISLDims}, {sendtime}) == {holder.getState()}"]
        constraints += [f"STATE({sendTensorName}, {sendexprISLDims}, {sendgexprISLDims}, {sendtime}) > 3"] ## has to be  own to copy 
    ...
    def defineAfterState(self, holder):
        ...
        constraints += [f"STATE({sendTensorName}, {sendexprISLDims}, {sendgexprISLDims}, {time}) == {holder.getState()}" ]
